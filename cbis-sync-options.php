...
function hk_cbis_update_products($returnlog = false) {
	...
	//echo "<br>Product search:<br>";
	//var_dump($product_search);
	
	$log .= "DEBUG: " . print_r($product_search, false) . " \n";
	$product_count = 0;
	if (!is_null($product_count)  && isset($product_count)) {
		$product_count = count($product_search);
	}
	$log .= "Hittade " . $product_count . " CBIS produkter, synkar till wordpress \n";
